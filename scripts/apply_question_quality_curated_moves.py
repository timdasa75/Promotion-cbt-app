#!/usr/bin/env python3
"""Apply a curated first batch of high-confidence question moves with ID renaming."""
...
MOVE_SPECS = [
    MoveSpec("fin_pro_023", "financial_regulations", "fin_procurement", "procurement_act", "proc_bidding_evaluation", "ppa_bid_"),
    MoveSpec("fin_pro_042", "financial_regulations", "fin_procurement", "procurement_act", "proc_implementation_sanctions", "ppa_ims_"),
    MoveSpec("ict_eg_024", "ict_management", "ict_e_governance", "financial_regulations", "fin_budgeting", "fin_bgt_"),
    MoveSpec("NGPD_004", "general_current_affairs", "ca_national_governance", "psr", "psr_appointments", "psr_app_"),
    MoveSpec("NGPD_006", "general_current_affairs", "ca_national_governance", "psr", "psr_appointments", "psr_app_"),
    MoveSpec("NGPD_023", "general_current_affairs", "ca_national_governance", "psr", "psr_leave", "psr_leave_"),
    MoveSpec("leadership_mpf_029", "leadership_management", "lead_management_performance", "psr", "psr_training", "psr_train_"),
]
...
def apply_moves(root: Path):
    ...
    for spec in MOVE_SPECS:
        source_subcategory = subcategory_refs[(spec.source_topic, spec.source_subcategory)]
        target_subcategory = subcategory_refs[(spec.target_topic, spec.target_subcategory)]
        source_questions = source_subcategory.get("questions") or []
        target_questions = target_subcategory.get("questions") or []

        source_index, question = find_question(source_subcategory, spec.question_id)
        if question is None:
        ...
        if "topic" in updated:
            updated["topic"] = str(topic_meta[spec.target_topic].get("name") or spec.target_topic).strip()
        updated["tags"] = normalize_tags(
            updated.get("tags"),
            spec.source_topic,
            spec.source_subcategory,
            spec.target_topic,
            spec.target_subcategory,
        )
...
def main() -> int:
    ...
    save_json(args.log_out, {"applied": applied})
