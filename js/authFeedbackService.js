import { normalizeEmail, normalizeFeedbackCategory, normalizeFeedbackSource, normalizeFeedbackStatus, toIsoTimestamp } from "./authNormalization.js";
...
async function workerRequest(path, body, accessToken) {
  try {
    return requestCloudflareAuth(path, { method: "POST", body, accessToken });
  } catch (e) {
...
export function getFeedbackAccessState({ currentUser = null, session = null, cloudAuthEnabled = false } = {}) {
  if (!currentUser?.email) {
    return { allowed: false, reason: "login-required", message: "Sign in to send feedback." };
  }
  if (!cloudAuthEnabled) {
    return { allowed: false, reason: "cloud-disabled", message: "Feedback is available only when online sign-in is configured." };
  }
  if (!session || !session.accessToken || session.provider === "local" || session.provider === "demo") {
    return { allowed: false, reason: "cloud-sign-in-required", message: "Feedback is available only when you sign in with an online account." };
  }
...
export function getFeedbackCooldownState(userId, { storage = localStorage, cooldownMs = FEEDBACK_SUBMIT_COOLDOWN_MS, nowMs = Date.now() } = {}) {
  const lastSubmittedAt = readFeedbackCooldown(userId, storage);
  const lastSubmittedMs = Date.parse(lastSubmittedAt || "");
  if (!lastSubmittedMs) return { lastSubmittedAt: "", remainingMs: 0 };
  const remainingMs = Math.max(0, Number(cooldownMs || 0) - Math.max(0, nowMs - lastSubmittedMs));
  return { lastSubmittedAt, remainingMs };
}

function normalizeFeedbackMessage(message) {
  const normalized = String(message || "").trim();
  if (!normalized) throw new Error("Feedback message is required.");
  if (normalized.length > FEEDBACK_MESSAGE_MAX_LENGTH) throw new Error(`Feedback message must be ${FEEDBACK_MESSAGE_MAX_LENGTH} characters or fewer.`);
  return normalized;
}
...
export async function submitFeedbackSubmission(input = {}, { cloudAuthEnabled = false, currentUser = null, session = null, refreshSession } = {}, { upsertFeedback, idFactory = () => `fbk_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`, now = () => new Date().toISOString(), storage = localStorage, cooldownMs = FEEDBACK_SUBMIT_COOLDOWN_MS } = {}) {
  ...
  const cooldown = getFeedbackCooldownState(userId, { storage, cooldownMs, nowMs: Date.now() });
  if (cooldown.remainingMs > 0) throw new Error(`Please wait ${Math.ceil(cooldown.remainingMs / 1000)} seconds before sending more feedback.`);
...
export async function getAdminFeedbackSubmissions({ cloudAuthEnabled = false, currentUserIsAdmin = false, session = null, refreshSession } = {}, { listFeedback, limit = 200 } = {}) {
  if (!currentUserIsAdmin) throw new Error("Admin access is restricted.");
  if (!cloudAuthEnabled) throw new Error("Feedback inbox requires an admin session.");
  ...
  if (listFeedback) return listFeedback(freshSession.accessToken, limit);
  if (typeof window === "undefined") return [];
  const result = await workerRequest("adminFeedbackList", { limit }, freshSession.accessToken);
...
export async function updateFeedbackSubmissionStatus(feedbackId, status, { cloudAuthEnabled = false, currentUserIsAdmin = false, session = null, refreshSession } = {}, { patchFeedback, now = () => new Date().toISOString() } = {}) {
  ...
  if (!["in_review", "resolved", "dismissed"].includes(nextStatus)) throw new Error("Invalid feedback status.");
  if (!currentUserIsAdmin) throw new Error("Admin access is restricted.");
  if (!cloudAuthEnabled) throw new Error("Feedback inbox requires an admin session.");
  if (typeof refreshSession !== "function") throw new Error("Admin session is unavailable.");
  const freshSession = await refreshSession(session, { clearOnFailure: true });
